...
| Realtime Assistant: tamanho das respostas e atalho de cópia | Respostas DEVEM ser sempre **ultra-curtas (1 a 2 linhas)** com termos em **negrito** (ex: `**DDD (Domain-Driven Design)** — Modelagem focada nas regras de negócio e domínio`). O contexto injeta apenas perguntas/tópicos recentes para não duplicar respostas anteriores. Sem botões poluídos na UI — o atalho `Ctrl+C` funciona nativamente sobre o texto selecionado na janela (`focusable: true` com `showInactive()`). |
