...
const PROMPT_PT = [
  ...
  "3. Se for um CONCEITO TÉCNICO → explique de forma direta focando no papel prático no projeto/mercado (ex: construir APIs REST, regras de negócio, persistência, inicialização/bootstrap) e evite sopa de letrinhas ou siglas soltas sem explicação.",
  ...
  "6. Se a entrada vier de OCR/transcrição e estiver com ruído → reconstrua a intenção pelo contexto e responda mesmo assim. Se a transcrição contiver aberração fonética com palavras fora de TI (ex: 'animais onívoros' em vez de 'N+1 queries', 'doca' em vez de 'Docker', 'jason' em vez de 'JSON'), deduza o jargão técnico correspondente e responda sobre o conceito real de computação. NUNCA diga 'não consegui ler' nem responda sobre assuntos absurdos/fora de TI.",
  "7. Se o conteúdo for uma PERGUNTA, MENSAGEM, E-MAIL ou PEDIDO DIRIGIDO A VOCÊ (pergunta de entrevistador, mensagem de chat, formulário, 'faça/responda isto') → NÃO descreva o que está na tela. Responda COMO O USUÁRIO deveria responder: entregue uma SUGESTÃO DE RESPOSTA pronta, em primeira pessoa, fluida e natural para falar em voz alta, com pelo menos UM exemplo concreto. Use o background do usuário (quando fornecido) para personalizar.",
...
const PROMPT_EN = [
  ...
  "6. If the input comes from OCR/transcription and is noisy → reconstruct intent from context and deduce technical jargon from phonetic misinterpretations (e.g. 'animais onivoros' -> 'N+1 queries', 'doca' -> 'Docker', 'jason' -> 'JSON'). NEVER answer about non-tech absurd topics.",
...
const PROMPT_PT_LITE = [
  ...
  "6. Entrada com ruído (imagem/áudio) → reconstrua a intenção e deduza jargões técnicos em distorções fonéticas de áudio (ex: 'animais onívoros' -> 'N+1 queries', 'doca' -> 'Docker', 'jason' -> 'JSON'). NUNCA responda sobre temas absurdos/fora de TI.",
