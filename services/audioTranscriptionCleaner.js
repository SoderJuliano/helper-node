...
const HALLUCINATION_PATTERNS = [
  ...
  /^(?:(?:sexta(?:\s+sal[ãa]o|-sal[ãa]o)?|sal[ãa]o(?:\s+de\s+festas|\s+nobre|\s+principal)?|sexta-feira|sextou)[\s.,!?:;]*)+$/i,
  /^(?:(?:arraste(?:\s+de\s+cadeira|\s+de\s+m[óo]veis)?|arrastar|arrastando|cadeira|porta|passos|vento|ar\s+condicionado|impacto|batida|barulho(?:\s+de\s+fundo|\s+de\s+cadeira)?|ru[íi]do(?:\s+de\s+fundo)?)[\s.,!?:;]*)+$/i,
  /^(?:(?:(?:muito\s+)?obrigad[oa](?:\s+a\s+todos|\s+pela\s+aten[çc][ãa]o)?|muito\s+obrigad[oa])[\s.,!?:;]*)+$/i,
  /^(?:(?:tchau\s+tchau|at[ée]\s+logo|at[ée]\s+mais|fui|am[ée]m)[\s.,!?:;]*)+$/i,
  /^(?:(?:[ée]\s+isso(?:\s+a[íi])?|pois\s+[ée])[\s.,!?:;]*)+$/i,
  /^(?:(?:assistente|nexa)[\s.,!?:;]*)+$/i,
  /^\[blank_audio\]$/i,
  /^\(sem\s+fala\)$/i,
];
