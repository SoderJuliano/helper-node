...
(function () {
  ...
  function clearAiDiffDecorations(doc) {
    if (!doc) return;
  ...
  function applyAiDiffDecorations(doc, previousContent, freshContent, origin) {
    clearAiDiffDecorations(doc);
    ...
    // Agrupa linhas contíguas em hunks
    const hunks = [];
    ...
    // Aplica classes de fundo e sublinhado
    ...
    // Cria a barra discreta embaixo de cada bloco alterado
    hunks.forEach(hunk => {
      ...
      acceptBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        clearAiDiffDecorations(doc);
        setSaveStatus('Alterações da IA aceitas ✓');
        setTimeout(() => setSaveStatus(''), 1500);
      });
      ...
      revertBtn.addEventListener('click', async (e) => {
        ...
        setSaveStatus('Revertido para o código original ✓');
    ...
    if (!doc.dirty) {
      ...
      if (filePath === activePath && cm) {
        if (cm.getValue() !== freshContent) {
          ...
          if (origin && origin !== 'disk' && origin !== 'user') {
            applyAiDiffDecorations(doc, previousContent, freshContent, origin);
          }
        }
